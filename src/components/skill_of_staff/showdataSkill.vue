                        ...
                        <table class="table table-hover table-striped" align="center">
                            ...
                            <tbody v-if="dataShow.length > 0">
                                <tr v-for="(data, i) in dataShow" :key="i">
                                    ...
                                    <button v-if="(data.id == id_user) || (idtype_user == 1)" href="" class="btn btn-danger btn-sm btn_xoa" @click.prevent="click_detroy(data.id)" :id="'btn_xoa'+data.id"><i class="fas fa-trash"></i> Delete </button>
                            </td>
...
<script>
    ...
    export default ({
      name: "showdataSkill",
        ...
        mounted() {
          this.id_user = JSON.parse(localStorage.getItem('user')).user.id;
                this.idtype_user = JSON.parse(localStorage.getItem('user')).user.id_type;
              this.checklogin();
        },
